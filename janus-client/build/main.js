...
/***/ function(module, exports, __webpack_require__) {
	...
	myApp.service('albumService', ['$location', __webpack_require__(17)]);
	myApp.service('videoService', ['$location', __webpack_require__(18)]);
	//custom directive to link our edit button to our template
	myApp.directive('editPhoto', ['$location', 'albumService', __webpack_require__(19)]);
	myApp.directive('editVideo', ['$location', 'videoService', __webpack_require__(20)]);
	//myApp.directive('deleteFile',['$location','albumService', require('./albums/deleteFile.js')]);
	myApp.directive('fileInput', ['$parse', __webpack_require__(21)]);
	myApp.directive('fileInputVideo', ['$parse', __webpack_require__(22)]);
	...
	myApp.config(['NgAdminConfigurationProvider', function (nga) {
	    ...
	    if (window.sessionStorage.getItem(_constants.LOCAL_SUPER_ADMIN) == "true") {
	        admin.menu((0, _menu.menu_superAdmin)(nga, admin));
	    } else if (window.sessionStorage.getItem(_constants.LOCAL_ADMIN) == "true") {
	        admin.menu((0, _menu.menu_admin)(nga, admin));
	    }
	    admin.header(__webpack_require__(31));
	    nga.configure(admin);
	}]);
...
/***/ function(module, exports) {
	...
	function menu_admin(nga, admin) {
		...
		return nga.menu().addChild(nga.menu(users).icon('<span class="fa fa-users fa-fw"></span>')).addChild(nga.menu(events).icon('<span class="fa fa-picture-o fa-fw"></span>')).addChild(nga.menu(albums).icon('<span class="fa fa-user-times fa-fw"></span>')).addChild(nga.menu(venues).icon('<span class="fa fa-institution fa-fw"></span>'));
	}
	...
	exports.menu_admin = menu_admin;
	exports.menu_superAdmin = menu_superAdmin;
	exports.menu_visitor = menu_visitor;
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.default = function (nga, admin) {
		...
		albums.showView().fields([nga.field('_id'), nga.field('name'), nga.field('title'), nga.field('description'), nga.field('date', 'datetime'), nga.field('Photos').template(__webpack_require__(28)) //<= we override this template
		]);
		//with our custom template albumsThumbnail.html, display inside the 'main' template
		// albums.showView().template("<album></album>"); 
		//we do the same thing
		albums.editionView().fields(albums.listView().fields(), nga.field('files').label('Photos').template(__webpack_require__(29)));
